...
static CBasePlayer* FindPlayerByName(const char *pTestName)
{
	...
	{
		edict_t *pEdict = g_engfuncs_h.pfnPEntityOfEntIndex(i);
		if(pEdict)
		{
...
{		  
	...
	g_engfuncs_h.pfnPrecacheModel("sprites/voiceicon.spr");
...
void CVoiceGameMgr::UpdateMasks()
{
	...
	for(int iClient=0; iClient < m_nMaxPlayers; iClient++)
	{
		...
		for(int iOtherClient=0; iOtherClient < m_nMaxPlayers; iOtherClient++)
		{
			...
			g_engfuncs_h.pfnVoice_SetClientListening(iClient+1, iOtherClient+1, bCanHear);
		}
	}
}
