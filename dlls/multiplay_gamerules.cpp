...
void CHalfLifeMultiplay::Think( void )
{
	...
	{
		g_engfuncs_h.pfnCvar_DirectSet( &fragsleft, UTIL_VarArgs( "%i", frags_remaining ) );
	}

	// Updates once per second
	if( timeleft.value != last_time )
	{
		g_engfuncs_h.pfnCvar_DirectSet( &timeleft, UTIL_VarArgs( "%i", time_remaining ) );
	}
...
void CHalfLifeMultiplay::InitHUD( CBasePlayer *pl )
{
	...
	if( g_teamplay )
	{
		UTIL_LogPrintf( "\"%s<%i><%s><%s>\" entered the game\n",  
			...
			g_engfuncs_h.pfnInfoKeyValue( g_engfuncs_h.pfnGetInfoKeyBuffer( pl->edict() ), "model" ) );
	}
	else
	{
		...
		if( pPlayer )
		{
			...
			if( g_teamplay )
			{
				UTIL_LogPrintf( "\"%s<%i><%s><%s>\" disconnected\n",  
					...
					g_engfuncs_h.pfnInfoKeyValue( g_engfuncs_h.pfnGetInfoKeyBuffer( pPlayer->edict() ), "model" ) );
			}
			else
			{
	...
	if( pVictim->pev == pKiller )  
	{
		...
		if( g_teamplay )
		{
			UTIL_LogPrintf( "\"%s<%i><%s><%s>\" committed suicide with \"%s\"\n",  
				...
				g_engfuncs_h.pfnInfoKeyValue( g_engfuncs_h.pfnGetInfoKeyBuffer( pVictim->edict() ), "model" ),
				killer_weapon_name );		
		}
		else
		{
	...
	else if( pKiller->flags & FL_CLIENT )
	{
		...
		if( g_teamplay )
		{
			UTIL_LogPrintf( "\"%s<%i><%s><%s>\" killed \"%s<%i><%s><%s>\" with \"%s\"\n",  
				...
				g_engfuncs_h.pfnInfoKeyValue( g_engfuncs_h.pfnGetInfoKeyBuffer( ENT(pKiller) ), "model" ),
				STRING( pVictim->pev->netname ),
				GETPLAYERUSERID( pVictim->edict() ),
				GETPLAYERAUTHID( pVictim->edict() ),
				g_engfuncs_h.pfnInfoKeyValue( g_engfuncs_h.pfnGetInfoKeyBuffer( pVictim->edict() ), "model" ),
	...
	if( pFileList && length )
	{
		...
		while( 1 )
		{
			...
			if( IS_MAP_VALID( szMap ) )
			{
				...
				if( hasbuffer )
				{
					s = g_engfuncs_h.pfnInfoKeyValue( szBuffer, "minplayers" );
					if( s && s[0] )
					{
					...
					s = g_engfuncs_h.pfnInfoKeyValue( szBuffer, "maxplayers" );
					if( s && s[0] )
					{
					...
					// Remove keys
					//
					g_engfuncs_h.pfnInfo_RemoveKey( szBuffer, "minplayers" );
					g_engfuncs_h.pfnInfo_RemoveKey( szBuffer, "maxplayers" );
...
void CMultiplayBusters::SetPlayerModel( CBasePlayer *pPlayer, BOOL bKnownBuster )
{
	...
	g_engfuncs_h.pfnSetClientKeyValue( ENTINDEX( pPlayer->edict()), g_engfuncs_h.pfnGetInfoKeyBuffer( pPlayer->edict()), "model", pszModel );
}
