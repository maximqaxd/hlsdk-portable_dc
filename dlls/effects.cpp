...
{ 
	...
	pev->owner = g_engfuncs_h.pfnPEntityOfEntIndex( entityIndex );
}

void CBeam::SetEndEntity( int entityIndex )
{ 
	pev->skin = ( entityIndex & 0x0FFF ) | ( pev->skin & 0xF000 ); 
	pev->aiment = g_engfuncs_h.pfnPEntityOfEntIndex( entityIndex );
}
...
const Vector &CBeam::GetStartPos( void )
{
	...
	{
		edict_t *pent =  g_engfuncs_h.pfnPEntityOfEntIndex( GetStartEntity() );
...
const Vector &CBeam::GetEndPos( void )
{
	...
	edict_t *pent =  g_engfuncs_h.pfnPEntityOfEntIndex( GetEndEntity() );
	if( pent )
		return pent->v.origin;
	return pev->angles;
}
...
Vector CBlood::BloodPosition( CBaseEntity *pActivator )
{
	if( pev->spawnflags & SF_BLOOD_PLAYER )
	{
		...
		if( pActivator && pActivator->IsPlayer() )
		{
			...
			pPlayer = g_engfuncs_h.pfnPEntityOfEntIndex( 1 );
		if( pPlayer )
...
void CMessage::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	else
	{
		if( pActivator && pActivator->IsPlayer() )
			...
			pPlayer = CBaseEntity::Instance( g_engfuncs_h.pfnPEntityOfEntIndex( 1 ) );
