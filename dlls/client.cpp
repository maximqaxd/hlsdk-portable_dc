...
void ClientPutInServer( edict_t *pEntity )
{
	...
	pPlayer->SetPrefsFromUserinfo( g_engfuncs_h.pfnGetInfoKeyBuffer( pEntity ) );
...
void Host_Say( edict_t *pEntity, int teamonly )
{
	...
	g_engfuncs_h.pfnServerPrint( text );
	...
	if( g_teamplay )
	{
		UTIL_LogPrintf( "\"%s<%i><%s><%s>\" %s \"%s\"\n", 
			...
			g_engfuncs_h.pfnInfoKeyValue( g_engfuncs_h.pfnGetInfoKeyBuffer( pEntity ), "model" ),
			temp,
			p );
	}
	else
...
void ClientUserInfoChanged( edict_t *pEntity, char *infobuffer )
{
	...
	if( pEntity->v.netname && ( STRING( pEntity->v.netname ) )[0] != 0 && !FStrEq( STRING( pEntity->v.netname ), g_engfuncs_h.pfnInfoKeyValue( infobuffer, "name" ) ) )
	{
		char sName[256];
		char *pName = g_engfuncs_h.pfnInfoKeyValue( infobuffer, "name" );
		...
		// Set the name
		g_engfuncs_h.pfnSetClientKeyValue( ENTINDEX( pEntity ), infobuffer, "name", sName );

		if( gpGlobals->maxClients > 1 )
		{
			char text[256];
			_snprintf( text, sizeof(text) - 1, "* %s changed name to %s\n", STRING( pEntity->v.netname ), g_engfuncs_h.pfnInfoKeyValue( infobuffer, "name" ) );
			text[sizeof(text) - 1] = '\0';
			MESSAGE_BEGIN( MSG_ALL, gmsgSayText, NULL );
				WRITE_BYTE( ENTINDEX( pEntity ) );
				WRITE_STRING( text );
		...
		if( g_teamplay )
		{
			UTIL_LogPrintf( "\"%s<%i><%s><%s>\" changed name to \"%s\"\n", 
				...
				GETPLAYERUSERID( pEntity ), 
				GETPLAYERAUTHID( pEntity ),
				g_engfuncs_h.pfnInfoKeyValue( infobuffer, "model" ), 
				g_engfuncs_h.pfnInfoKeyValue( infobuffer, "name" ) );
		}
		else
		{
