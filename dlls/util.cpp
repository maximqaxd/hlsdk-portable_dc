...
edict_t *DBG_EntOfVars( const entvars_t *pev )
{
	...
	edict_t *pent = (*g_engfuncs_h.pfnFindEntityByVars)( (entvars_t*)pev );
	if( pent == NULL )
...
{
	g_engfuncs_h.pfnTraceModel( vecStart, vecEnd, hullNumber, pentModel, ptr );
}
...
void EntvarsKeyvalue( entvars_t *pev, KeyValueData *pkvd )
{
	...
	for( i = 0; i < (int)ENTVARS_COUNT; i++ )
	{
		...
		if( !stricmp( pField->fieldName, pkvd->szKeyName ) )
		{
			...
			case FIELD_STRING:
#if XASH_DREAMCAST // unaligned access fix
				{
		...
		{			
			if( !m_global || !(pTest->flags & FTYPEDESC_GLOBAL ) )
			{
				for( j = 0; j < pTest->fieldSize; j++ )
				{
					...
					{
					case FIELD_TIME:
#if __VFP_FP__  || XASH_DREAMCAST // unaligned access fix
